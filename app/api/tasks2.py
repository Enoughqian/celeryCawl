...
# 异步接口
async def afetch_product_info(url: str, browser, context):
    """
    获取异步接口数据
    """
    ...
    async with async_playwright() as playwright:
        ...
        # 调用火狐浏览器
        browser = await playwright.firefox.launch(headless = if_headless)
...
async def search_product(product_id):
    ...
    result = await afetch_product_info(url, None, None)
    if result:
...
if __name__ == "__main__":
    ...
    asyncio.run(search_product(product_id))